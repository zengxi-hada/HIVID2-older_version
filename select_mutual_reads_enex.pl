...
while(<HBV>){
	...
	my $hbv_fqseq = $a[9];
	if($hbv_cigar =~ /\*/){next;}
	next if($hbv_cigar =~ /H/);		# 2019-07-02, next if there are hardclip because hardclip is always worse than softclip for the same read.
	...
	if(not exists $mutual{$id}){
		...
		if($mapq > (split /\s+/,$mutual{$id})[1]){
			...
			if(exists $record_hbv{$id}){
				...
				push @{$tmp_hash{$id}}, "$id\_$count1{$id}";									# reads with more than one hits
				$record_hbv{$id} = 1;
				$count1{$id}++;
		}
	}
...
while(<HUMAN>){
	...
	my $id = $a[0];
	if($a[5] =~ /\*/){next;}
#	my $flag_hbv_cigar = 0;				# added 2019-07-02
	my $human_cigar = $a[5];
	next if $human_cigar =~ /H/; 		# 2019-07-02, next if there are hardclip because hardclip is always worse than softclip for the same read.
	if (exists $mutual{$id}){
		my @array_inter;
...
####################################
		...
		$mhbv += $1 while($hbv_cigar =~ /(\d+)M/g);				# mhbv is length of matched bases with virus genome
		$mhuman += $1 while($human_cigar =~ /(\d+)M/g);			# mhuman is length of matched bases with human genome
		
		next if ($hbv_cigar!~/S/ || $mhbv<15);					# next if no softclip in hbv cigar
		next if ($human_cigar!~/S/ || $mhuman<15);				# next if no softclip in human cigar 
#		$flag_hbv_cigar = 1 if($hbv_cigar = ~/H/);			# added at 2019-07-02
				
#		print "$hbv_cigar\t$human_cigar\n";
		...
		for my $i1(@hbv_array_pos){
			...
			for my $i2(@human_array_pos){
				...
				if (($dvalue1 > 15) && ($dvalue2 > 15)){					## modiify at 2019/08/15 (allow the homology seq lower than 20bp)
...
for my $key1(keys %print_human){
	for my $key2(keys %{$print_human{$key1}}){
		...
		}else{																			#for the cases where the reads are repeatly mapped on HBV genome
			for my $k1(keys %print_hbv){
				next if $k1 ne $key1;
				for my $k2(keys %{$print_hbv{$k1}}){									# k1 is key1
					my $k2_tmp = $k2;														# modify at 20190905
					$k2_tmp =~ s/_\d+$//;
					if($key2 =~ /$k2_tmp/){
						$hbv_mapq = (split /\t/, $print_hbv{$key1}{$k2})[-1];			# select one hbv mapq as this read is repeatly mapped in several positions with the same mapq
						last;															# only one is ok as all mapqs for this read are the same.
					}
				}
			}
		}
