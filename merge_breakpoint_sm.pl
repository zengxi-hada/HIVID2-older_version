...
#######################################################################################################
### this program is to merge breakpoints with distance less than 20bps for the host gneome  ###########
### Author: Zeng Xi																			###########
### Last Update: 2020-12																	###########
#######################################################################################################
...
while(<IN>){
	...
	for my $i($pos-$distance..$pos+$distance){
		if(exists $record_pos{$ref}{$i}){
			$flag2 = 1;
			push @tmp_record, "$ref\t$pos\t$sum_sup";		#if there are breakpoints within the distance, the information of each bk will be recorded
#			print "mark0  $ref\t$pos\t$sum_sup\n";
#			print "ref  $ref\n";
#			print "pos  $pos\n";
#			print "sum_sup  $sum_sup\n";
			last;
		}
	}
	if($flag2 == 0){
		...
		if(@tmp_record != 0){
			$max_item =  &max_sup (@tmp_record);			# the bk with maxmum support reads will be reserved for breakpoints within the distance
			my $iref = (split /\t/, $max_item)[0];
            my $ipos = (split /\t/, $max_item)[1];
            my $isup = (split /\t/, $max_item)[2];			# the support reads of the bk with the maximum support reads will be used
