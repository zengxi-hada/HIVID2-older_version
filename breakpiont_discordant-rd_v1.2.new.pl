...
#    my ($chr,$start)=(split /\s+/)[1,2];													# $start is the alignment position of discordant reads on human genome
	...
	for my $pos (keys %{$bk{$chr}}){
        if($start<=$pos+$len && $start>=$pos-$len){		# acutally, the distance should be lib_len-read_len, but consider the deviation of li_len, here we only use lib_len
	...
	if($flag == 0){																			# it is the situation that all breakpoints were not close to the discordant_pos
		my $pair_id = $id; $pair_id =~ s/\/\d//;
		print ODS "discordant_pos:$chr-$start\tno_close_bk_in_split_read\t0\n";					# print out this situation
...
while (<BK>){
	...
	if(not exists $disc_bk{"$chr\t$pos"}){									# there were no discordant reads mapped on the bk identified by split reads
		print OBK "$a[0]\t$a[1]\t$a[2]\t$a[3]\t0\t$a[4]\t$a[5]\t$a[6]\t0\t$a[7]\t$a[8]\t$a[9]\t0\n";
	}else{
		...
		print OBK "$a[0]\t$a[1]\t$a[2]\t$a[3]\t$count\t$new_sum_sup\t$a[5]\t$a[6]\t0\t$new_norm_sum\t$a[8]\t$a[9]\t$reads_str\n";
...
for my $chr(keys %disc_bk_nosplit){
	for my $pos(keys %{$disc_bk_nosplit{$chr}}){								#print out merged bk only supported by discordant reads
		...
		print OBK "$chr\t$pos\t0\t0\t$count\t$count\t0\t0\t$new_norm_sum\t$new_norm_sum\t0\t0\t$reads_str\n";
	}
}
close OBK;
