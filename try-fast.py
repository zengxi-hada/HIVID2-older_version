	...
	with open (cmd_args.o,"w") as outfile:
		for line in infile:
			...
			if line[0] !="ref":
				...
				total_support=str(uniq_read1+uniq_read2)
				norm_left=str(round(uniq_read1/float(line[2])*float(line[5]),3)) if uniq_read1>0 else "0"
				norm_right=str(round(uniq_read2/float(line[3])*float(line[6]),3)) if uniq_read2>0 else "0"
				norm_sum=str(round(float(norm_left)+float(norm_right),3))
				...
				outfile.write(output1+"\t"+left_support+"\t"+right_support+"\t"+total_support+"\t"+norm_left+"\t"+norm_right+"\t"+norm_sum+"\t"+left_reads_ID+"\t"+right_reads_ID+"\n")
			else:
