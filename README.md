...
(2) About setting the length in sample.list: It is OK to set the length based on the raw reads, But it will be better set the reads length after quality control. Actually, users can set the read length in sample.list after completing step2 because this value of length will not be used in step2 but used in step3. And in step2, adapters will be removed. Please note that the length of each read will be not the same after removing adapter. So You can use a length with highest frequency as the length set in sample.list. For example, if there are five reads in total and the read length after removing adapter are: 100, 95, 100, 100, 100, then 100 should be set as the length in sample.list. You can refer to the file "*.trimmo.paired.gz" (fq1 or fq2) in step2 for estimating the reads length.
