...
##      $d[0]=~s/[12]$//;
		...
		my $fq2_str = $id_fq2_str.$seq_fq2.$line3_fq2.$qual_fq2;
...
sub read_pese{
	...
	while(<IN>){
		chomp;
		my ($soap_id, $unq, $read_order, $oritation, $chr, $align_pos) = (split /\s+/)[0, 3, 4, 6, 7, 8];
		if($unq>1 and $ter eq "human"){						# filter non-uniq alignment
			next;
		}
		my $read_order_ts = ($read_order eq "a") ? "1" : "2";
		$$hash{$soap_id}{$read_order_ts}="$chr\t$align_pos\t$oritation";
	}
	close IN;
}
